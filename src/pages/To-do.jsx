...
// Выносим массив объектов для удобного управления
const initialTasks = [
    { id: 'task-1', text: 'Code Review', completed: false, type: 'task' },
    { id: 'task-2', text: 'Learn Redux Toolkit', completed: false, type: 'task' },
    { id: 'task-3', text: 'Daily Workout', completed: false, type: 'task' },
];
...
export default function ToDoList() {
    ...
    // Функция переключения статуса с начислением XP
    const toggleTask = (id) => {
        ...
        // Начисляем XP только если задача была не завершена (завершаем её)
        if (wasNotCompleted) {
            dispatch(addXp(10));
        }
    };
                        ...
                        >
                            <div className="relative flex items-center justify-center w-5 h-5">
                                ...
                                <div className={`w-full h-full rounded border-2 flex items-center justify-center transition-colors ${
                                    task.completed ? 'bg-game-purple border-game-purple' : 'border-game-purple/50'
                                }`}>
                                    {task.completed && (
                                        <motion.svg 
                                            initial={{ scale: 0 }}
                            ...
                            <span className={`flex-1 transition-all duration-500 ${
                                task.completed ? 'text-game-purple/50 line-through decoration-game-pink italic' : 'text-white'
                            }`}>
                                {task.text}
                            </span>
                        </motion.li>
